<?php
    ...
    try{
        ...
        if($order == 1) {
            $query = $db->prepare("SELECT permanent_users.surname, permanent_users.firstname, issues.subject, issues.issue, issues.date_time, issues.unique_id
                  FROM permanent_users INNER JOIN issues ON permanent_users.user_id=issues.user_id order by surname asc ");
        }
        else if($order == 2){
            $query = $db->prepare("SELECT permanent_users.surname, permanent_users.firstname, issues.subject, issues.issue, issues.date_time, issues.unique_id
                  FROM permanent_users INNER JOIN issues ON permanent_users.user_id=issues.user_id order by surname desc ");
        }
        else if($order == 3){
            $query = $db->prepare("SELECT permanent_users.surname, permanent_users.firstname, issues.subject, issues.issue, issues.date_time, issues.unique_id
                  FROM permanent_users INNER JOIN issues ON permanent_users.user_id=issues.user_id order by firstname asc ");
        }
        else if($order == 4){
            $query = $db->prepare("SELECT permanent_users.surname, permanent_users.firstname, issues.subject, issues.issue, issues.date_time, issues.unique_id
                  FROM permanent_users INNER JOIN issues ON permanent_users.user_id=issues.user_id order by firstname desc ");
        }
        else if($order == 5){
            $query = $db->prepare("SELECT permanent_users.surname, permanent_users.firstname, issues.subject, issues.issue, issues.date_time, issues.unique_id
                  FROM permanent_users INNER JOIN issues ON permanent_users.user_id=issues.user_id order by date_time asc ");
        }
        else if($order == 6){
            $query = $db->prepare("SELECT permanent_users.surname, permanent_users.firstname, issues.subject, issues.issue, issues.date_time, issues.unique_id
                  FROM permanent_users INNER JOIN issues ON permanent_users.user_id=issues.user_id order by date_time desc ");
        }
        $query->execute();
