<?php
    ...
    try{
        ...
        if($order == 1) {
            $query = $db->prepare("SELECT permanent_users.surname, permanent_users.firstname, ratings.rating, ratings.date_time, ratings.unique_id
          FROM permanent_users INNER JOIN ratings ON permanent_users.user_id=ratings.user_id order by surname asc ");
        }
        else if($order == 2){
            $query = $db->prepare("SELECT permanent_users.surname, permanent_users.firstname, ratings.rating, ratings.date_time, ratings.unique_id
          FROM permanent_users INNER JOIN ratings ON permanent_users.user_id=ratings.user_id order by surname desc ");
        }
        else if($order == 3){
            $query = $db->prepare("SELECT permanent_users.surname, permanent_users.firstname, ratings.rating, ratings.date_time, ratings.unique_id
          FROM permanent_users INNER JOIN ratings ON permanent_users.user_id=ratings.user_id order by firstname asc ");
        }
        else if($order == 4){
            $query = $db->prepare("SELECT permanent_users.surname, permanent_users.firstname, ratings.rating, ratings.date_time, ratings.unique_id
          FROM permanent_users INNER JOIN ratings ON permanent_users.user_id=ratings.user_id order by firstname desc ");
        }
        else if($order == 5){
            $query = $db->prepare("SELECT permanent_users.surname, permanent_users.firstname, ratings.rating, ratings.date_time, ratings.unique_id
          FROM permanent_users INNER JOIN ratings ON permanent_users.user_id=ratings.user_id order by rating asc ");
        }
        else if($order == 6){
            $query = $db->prepare("SELECT permanent_users.surname, permanent_users.firstname, ratings.rating, ratings.date_time, ratings.unique_id
          FROM permanent_users INNER JOIN ratings ON permanent_users.user_id=ratings.user_id order by rating desc ");
        }
        else if($order == 7){
            $query = $db->prepare("SELECT permanent_users.surname, permanent_users.firstname, ratings.rating, ratings.date_time, ratings.unique_id
          FROM permanent_users INNER JOIN ratings ON permanent_users.user_id=ratings.user_id order by date_time asc ");
        }
        else if($order == 8){
            $query = $db->prepare("SELECT permanent_users.surname, permanent_users.firstname, ratings.rating, ratings.date_time, ratings.unique_id
          FROM permanent_users INNER JOIN ratings ON permanent_users.user_id=ratings.user_id order by date_time desc ");
        }
        $query->execute();
