...
$pwd = 'workchop_12345';
    ...
    try{
        ...
        if($order == 1) {
            $query = $db->prepare("SELECT permanent_vendors.vendor_name, issues_vendor.subject, issues_vendor.issue, issues_vendor.date_time, issues_vendor.unique_id
                      FROM permanent_vendors INNER JOIN issues_vendor ON permanent_vendors.vendor_id=issues_vendor.vendor_id order by vendor_name asc ");
        }
        else if($order == 2){
            $query = $db->prepare("SELECT permanent_vendors.vendor_name, issues_vendor.subject, issues_vendor.issue, issues_vendor.date_time, issues_vendor.unique_id
                      FROM permanent_vendors INNER JOIN issues_vendor ON permanent_vendors.vendor_id=issues_vendor.vendor_id order by vendor_name desc ");
        }
        else if($order == 3){
            $query = $db->prepare("SELECT permanent_vendors.vendor_name, issues_vendor.subject, issues_vendor.issue, issues_vendor.date_time, issues_vendor.unique_id
                      FROM permanent_vendors INNER JOIN issues_vendor ON permanent_vendors.vendor_id=issues_vendor.vendor_id order by date_time asc ");
        }
        else if($order == 4){
            $query = $db->prepare("SELECT permanent_vendors.vendor_name, issues_vendor.subject, issues_vendor.issue, issues_vendor.date_time, issues_vendor.unique_id
                      FROM permanent_vendors INNER JOIN issues_vendor ON permanent_vendors.vendor_id=issues_vendor.vendor_id order by date_time desc ");
        }
        $query->execute();
        ...
        echo "
                        <table border=\"3px\" cellpadding=\"3px\" cellspacing=\"4px\" style=\"border-color: #2b669a; font-family: 'Century Gothic';\">
                            <tr style=\"font-size: 15px; font-style: normal;font-weight: normal;\">
                                ...
                                <th style=\"font-weight: normal;padding: 0px 7px 0px 7px; \">Vendor Name</th>
